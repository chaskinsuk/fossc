...
if ( ! class_exists( 'Corporate_Club_About' ) ) {
	...
	class Corporate_Club_About {
		...
		/**
		 * Singleton instance of Corporate_Club_About.
		 *
		 * @var Corporate_Club_About $instance Corporate_Club_About instance.
		 */
		private static $instance;

		/**
		 * Main Corporate_Club_About instance.
		 *
		 * @since 1.0.0
		 *
		...
		public static function init( $config ) {
			if ( ! isset( self::$instance ) && ! ( self::$instance instanceof Corporate_Club_About ) ) {
				self::$instance = new Corporate_Club_About;
		...
		public function setup_config() {
			...
			$dismiss_url = wp_nonce_url( add_query_arg( array( 'corporate-club-at-dismiss' => '1' ), admin_url( 'themes.php' ) ), 'corporate-club-action-dismiss-' . get_current_user_id(), 'corporate-club-dismiss-nonce' );
			$this->notification  = isset( $this->config['notification'] ) ? $this->config['notification'] : ( '<p>' . sprintf( esc_html__( 'Welcome! Thank you for choosing %1$s! To fully take advantage of the best our theme can offer please make sure you visit our %2$swelcome page%3$s.', 'corporate-club' ), $this->theme_name, '<a href="' . esc_url( admin_url( 'themes.php?page=' . $this->page_slug ) ) . '">', '</a>' ) . '</p><p><a href="' . esc_url( admin_url( 'themes.php?page=' . $this->page_slug ) ) . '" class="button button-primary" style="text-decoration: none;">' . sprintf( esc_html__( 'Get started with %s', 'corporate-club' ), $this->theme_name ) . '</a>' ) . '<a href="' . esc_url( $dismiss_url ) . '" class="btn-at-dismiss">' . esc_html__( 'Dismiss', 'corporate-club' ) . '</a></p>';
		}
		...
		function hide_notice() {
			// Verify nonce.
			if (! ( isset( $_GET['corporate-club-dismiss-nonce'] ) && wp_verify_nonce( sanitize_key( $_GET['corporate-club-dismiss-nonce'] ), 'corporate-club-action-dismiss-' . get_current_user_id() ) ) ) {
				return;
			}

			if ( 1 === absint( $_GET['corporate-club-at-dismiss'] ) ) {
				update_user_meta( get_current_user_id(), 'corporate_club_at_dismissed', 1 );
			}
		}
		...
		public function activation_admin_notice() {
			...
			$dismissed_status = get_user_meta( get_current_user_id(), 'corporate_club_at_dismissed', true );
		...
		public function load_assets( $hook ) {
			...
			if ( 'appearance_page_' . $this->page_slug === $hook ) {
				...
				wp_enqueue_style( 'corporate-club-about', get_template_directory_uri() . '/vendors/about/css/about' . $min . '.css', array(), '1.0.1' );
				wp_enqueue_script( 'corporate-club-about', get_template_directory_uri() . '/vendors/about/js/about' . $min . '.js', array( 'jquery' ), '1.0.1' );
				$js_vars = array(
					'ajaxurl' => esc_url( admin_url( 'admin-ajax.php' ) ),
				);
				wp_localize_script( 'corporate-club-about', 'corporateClubAboutObject', $js_vars );
			}
		}
						...
						if ( ! empty( $action_value['plugin_slug'] ) ) {
							...
							switch ( $active['needs'] ) {
								...
								case 'install':
									...
									$label = esc_html__( 'Install', 'corporate-club' );
									break;
								case 'activate':
									$class = 'activate-now button button-primary';
									$label = esc_html__( 'Activate', 'corporate-club' );
									break;
								case 'deactivate':
									$class = 'deactivate-now button';
									$label = esc_html__( 'Deactivate', 'corporate-club' );
									break;
							}
							?>
		...
		public function upgrade_to_pro() {
			...
			if ( isset( $upgrade_to_pro['button_link'] ) && ! empty( $upgrade_to_pro['button_link'] ) ) {
				$button_text = esc_html__( 'Upgrade to Pro', 'corporate-club' );
		...
		public function useful_plugins() {
			...
			if ( isset( $tgmpa ) ) {
				...
				if ( true === $complete ) {
					echo '<div class="all-active"><span class="dashicons dashicons-yes"></span>&nbsp;' . esc_html__( 'Great, all recommended plugins are intalled and active.', 'corporate-club' ) . '</div>';
				}
				...
				if ( true !== $complete ) {
					echo '<div class="button-wrap"><a href="' . esc_url( $url ) . '" class="button button-primary">' . esc_html__( 'Manage Plugins', 'corporate-club' ) . '</a></div>';
				}
